...
//		js_pane.getViewport().setBorder(null);
	...
	public void setDefaultRenderer(TDefaultTableCellRenderer tdcr) {
		webTable.setDefaultRenderer(TSEntry.class, tdcr);
...
//			tdcr.setIconParameters(0, "document", null);
	...
	private void setTableColumns() {
		...
		String[] cName = tableColumns.split(";");
		Vector<String> tableCols = new Vector<>(columns.keySet());
		DefaultTableColumnModel dtcm = new DefaultTableColumnModel();
		// TODO: check for error in columnname??
		// Model mod = tableModel.getModel();
		for (int col = 0; col < cName.length; col++) {
			// TODO: put columwith using the 2 argument contstructor
			TableColumn tc = new TableColumn(tableCols.indexOf(cName[col]));
			String en = TStringUtils.getString(cName[col]);
