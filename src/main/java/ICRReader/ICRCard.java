...
public class ICRCard implements Comparable<ICRCard> {
	...
	public ICRCard(String val) {
		if (val.length() < 2) {
			// this error produce an iterruption in the caller method that avoid, this hand
			// to be added (error en pdb/pdb.playername files)
			// System.out.println("terry: error in card information.");
		}
	...
	/**
	 * parse the string representation and return a list of cards. the num argument
	 * control the numer of card to return.
	 * -1 retunr a list with all cards. 0 return empty list, 1 return a list only
	 * with the first cart and so on
	 * 
