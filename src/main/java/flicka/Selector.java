...
public class Selector {
	...
	private void takeDecision(Vector<PDistribution> pdList) {
		...
		// step 2
		ArrayList<TEntry<PDistribution, Double>> values = new ArrayList<>();
		for (PDistribution r : pdList) {
			TEntry<PDistribution, Double> te = new TEntry<>(r, r.getDouble("pdprediction")); 
			values.add(te);
		}
		Collections.sort(values);
		// in hitcount may be less element that selrance
		int sis = values.size() < selRange ? values.size() : selRange;
		for (int i = 0; i < sis; i++) {
			// for (int i = 0; i < vals.size(); i++) {
			TEntry<PDistribution, Double> te = (TEntry<PDistribution, Double>) values.get(i);
