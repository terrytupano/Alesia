...
public class TDockingContainer extends WebPanel {
	...
	public void home(ActionEvent event) {
		cmpList.removeAllElements();
		showPanel(homePanel, Direction.right);
	}

	@org.jdesktop.application.Action
	public void previous(ActionEvent event) {
		cmpList.remove(cmpCounter);
		JComponent cmp = cmpList.elementAt(--cmpCounter);
		cmpList.remove(cmp);
	...
	private void syncNavActions() {
		previous.setEnabled(cmpList.size() > 1);
		home.setEnabled(cmpList.size() > 1);
	}
}
