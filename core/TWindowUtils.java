...
public class TWindowUtils {
	...
	/**
	 * This class forces a heavyweight popup on the parent {@link Window}. See the implementation of
	 * {@link PopupFactory}; a heavyweight is forced if there is an occluding subwindow on the target window.
	 * <p>
	 * Ideally we'd have more control over {@link PopupFactory} but this is a fairly simple, lightweight workaround.
	 * Note that, at least as of JDK 1.6, the following do not have the desired effect:<br>
	 * 
	 * <pre>
	 * <code>
	...
	/**
	 * This can be installed over a {@link JLayeredPane} in order to listen for repaint requests. The content's repaint
	 * method will be invoked whenever any part of the ancestor window is repainted.
	 */
	...
	public static abstract class NativeWindowUtils {
		...
		/**
		 * Set the window to be transparent. Only explicitly painted pixels will be non-transparent. All pixels will be
		 * composited with whatever is under the window using their alpha values.
		 */
		...
		/**
		 * Set the window mask based on the given Raster, which should be treated as a bitmap (zero/nonzero values
		 * only). A value of <code>null</code> means to remove the mask.
		 */
		...
		/**
		 * Use this method to ensure heavyweight popups are used in conjunction with a given window. This prevents the
		 * window's alpha setting or mask region from being applied to the popup.
		 */
		 ...
		 *
		 * @param hwnd The concerning window handle.
		 * @return Either the window's icon or {@code null} if an error occurred.
		 *
		 * @throws UnsupportedOperationException Thrown if this method wasn't yet implemented for the current platform.
		 */
		 ...
		 *
		 * @param hIcon The icon handle type.
		 * @return Either the requested icon's dimension or an {@link Dimension} instance of {@code (0, 0)}.
		 *
		 * @throws UnsupportedOperationException Thrown if this method wasn't yet implemented for the current platform.
		 */
		 ...
		 *
		 * @param onlyVisibleWindows Specifies whether only currently visible windows will be considered ({@code true}).
		 *        That are windows which are not minimized. The {@code WS_VISIBLE} flag will be checked (see:
		 *        <a href= "https://msdn.microsoft.com/de-de/library/windows/desktop/ms633530%28v=vs.85%29.aspx"
		 *        >TUser32.IsWindowVisible(HWND)</a>).
		 *
		 ...
		 *
		 * @param hwnd The concerning window handle.
		 * @return Either the title or an empty string of no title was found or an error occurred.
		 *
		 * @throws UnsupportedOperationException Thrown if this method wasn't yet implemented for the
		 */
		 ...
		 *
		 * @param hwnd The concerning window handle for which the PE file path is required.
		 * @return The full file path of the PE file that is associated with the specified window handle.
		 *
		 * @throws UnsupportedOperationException Thrown if this method wasn't yet implemented for the
		 */
	...
	private static class Holder {
		/**
		 * Indicates whether a window must be visible before its native handle can be obtained. This wart is caused by
		 * the Java 1.4/X11 implementation.
		 */
	...
	private static class W32WindowUtils extends NativeWindowUtils {
		...
		/**
		 * Keep track of the alpha level, since we can't read it from the window itself.
		 */
		...
		/**
		 * W32 makes the client responsible for repainting the <em>entire</em> window on any change. It also does not
		 * paint window decorations when the window is transparent.
		 */
		...
		/**
		 * Note that w32 does <em>not</em> paint window decorations when the window is transparent.
		 */
		@Override
		...
		public BufferedImage getWindowIcon(final HWND hwnd) {
			...
			LRESULT result = TUser32.INSTANCE.SendMessageTimeout(hwnd, WinUser.WM_GETICON, new WPARAM(WinUser.ICON_BIG),
					new LPARAM(0), WinUser.SMTO_ABORTIFHUNG, 500, hIconNumber);
			if (result.intValue() == 0)
				result = TUser32.INSTANCE.SendMessageTimeout(hwnd, WinUser.WM_GETICON, new WPARAM(WinUser.ICON_SMALL),
						new LPARAM(0), WinUser.SMTO_ABORTIFHUNG, 500, hIconNumber);
			if (result.intValue() == 0)
				result = TUser32.INSTANCE.SendMessageTimeout(hwnd, WinUser.WM_GETICON, new WPARAM(WinUser.ICON_SMALL2),
						new LPARAM(0), WinUser.SMTO_ABORTIFHUNG, 500, hIconNumber);
			if (result.intValue() == 0) {
				result = new LRESULT(TUser32.INSTANCE.GetClassLongPtr(hwnd, WinUser.GCLP_HICON).intValue());
				hIconNumber.getValue().setValue(result.intValue());
			}
			if (result.intValue() == 0) {
				result = new LRESULT(TUser32.INSTANCE.GetClassLongPtr(hwnd, WinUser.GCLP_HICONSM).intValue());
				hIconNumber.getValue().setValue(result.intValue());
			}
			if (result.intValue() == 0)
				return null;
			...
			final HDC hDC = TUser32.INSTANCE.GetDC(null);
			final ICONINFO iconInfo = new ICONINFO();
			TUser32.INSTANCE.GetIconInfo(hIcon, iconInfo);
			iconInfo.read();
			...
			TUser32.INSTANCE.ReleaseDC(null, hDC);
		...
		public Dimension getIconSize(final HICON hIcon) {
			final ICONINFO iconInfo = new ICONINFO();
			try {
				if (!TUser32.INSTANCE.GetIconInfo(hIcon, iconInfo))
		...
		public List<DesktopWindow> getAllWindows(final boolean onlyVisibleWindows) {
			...
			final WNDENUMPROC lpEnumFunc = new WNDENUMPROC() {
				...
				public boolean callback(final HWND hwnd, final Pointer arg1) {
					try {
						final boolean visible = !onlyVisibleWindows || TUser32.INSTANCE.IsWindowVisible(hwnd);
						if (visible) {
			...
			if (!TUser32.INSTANCE.EnumWindows(lpEnumFunc, null))
		...
		public String getWindowTitle(final HWND hwnd) {
			final int requiredLength = TUser32.INSTANCE.GetWindowTextLength(hwnd) + 1;
			final char[] title = new char[requiredLength];
			final int length = TUser32.INSTANCE.GetWindowText(hwnd, title, title.length);
		...
		public String getProcessFilePath(final HWND hwnd) {
			...
			TUser32.INSTANCE.GetWindowThreadProcessId(hwnd, pid);

			final HANDLE process = Kernel32.INSTANCE
					.OpenProcess(WinNT.PROCESS_QUERY_INFORMATION | WinNT.PROCESS_VM_READ, false, pid.getValue());
			if (process == null) {
		...
		public Rectangle getWindowLocationAndSize(final HWND hwnd) {
			...
			if (!TUser32.INSTANCE.GetWindowRect(hwnd, lpRect))
				throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
	...
	private static class MacWindowUtils extends NativeWindowUtils {
		...
		/**
		 * Note that the property <code>apple.awt.draggableWindowBackground</code> must be set to its final value
		 * <em>before</em> the heavyweight peer for the Window is created. Once {@link Component#addNotify} has been
		 * called on the component, causing creation of the heavyweight peer, changing this property has no effect.
		 * 
		 * @see <a href=
		 *      "http://developer.apple.com/technotes/tn2007/tn2196.html#APPLE_AWT_DRAGGABLEWINDOWBACKGROUND">Apple
		 *      Technote 2007</a>
		 */
		@Override
		public void setWindowTransparent(Window w, boolean transparent) {
			...
			Runnable action = new Runnable() {
				...
				public void run() {
					...
					try {
						...
						ext.XShapeCombineMask(dpy, win, X11.Xext.ShapeBounding, 0, 0, pm == null ? Pixmap.None : pm,
								X11.Xext.ShapeSet);
					} finally {
						if (pm != null) {
	...
	/**
	 * Applies the given mask to the given window. Does nothing if the operation is not supported. The mask is treated
	 * as a bitmap and ignores transparency.
	 */
	...
	/**
	 * Applies the given mask to the given heavyweight component. Does nothing if the operation is not supported. The
	 * mask is treated as a bitmap and ignores transparency.
	 */
	...
	/**
	 * Set the overall window transparency. An alpha of 1.0 is fully opaque, 0.0 fully transparent. The alpha level is
	 * applied equally to all window pixels.
	 * <p>
	 * NOTE: Windows requires that <code>sun.java2d.noddraw=true</code> in order for alpha to work.
	 * <p>
	 * NOTE: On OSX, the property <code>apple.awt.draggableWindowBackground</code> must be set to its final value
	 * <em>before</em> the heavyweight peer for the Window is created. Once {@link Component#addNotify} has been called
	 * on the component, causing creation of the heavyweight peer, changing this property has no effect.
	 * 
	 ...
	 * composited with whatever is under the window using their alpha values.
	 *
	 * On OSX, the property <code>apple.awt.draggableWindowBackground</code> must be set to its final value
	 * <em>before</em> the heavyweight peer for the Window is created. Once {@link Component#addNotify} has been called
	 * on the component, causing creation of the heavyweight peer, changing this property has no effect.
	 * 
	 * @see <a href="http://developer.apple.com/technotes/tn2007/tn2196.html#APPLE_AWT_DRAGGABLEWINDOWBACKGROUND">Apple
