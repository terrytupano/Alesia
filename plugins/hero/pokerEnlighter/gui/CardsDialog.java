...
public class CardsDialog extends JDialog
{
            ...
            for (int j = 0; j < 13; j++) {
                if (! labelStates[i][j]) {
                    cardsLabels[i][j] = new JLabel(
                        new ImageIcon (getClass().getResource ("../resources/blank.card.gif"))
                    );
                } else {
                    char color = '.';
                    ...
                    cardsLabels[i][j] = new JLabel(
                        new ImageIcon(getClass().getResource("../resources/cards/" + Card.getCharCard(j + 2) + color + ".gif"))
                    );
                }
    ...
    private JPanel createSelectedPanel(int nrOfRequestedCards, Card[] alreadySelectedCards)
    {
        ...
        for (int i = 0; i < nrOfRequestedCards; i++) {
            if (alreadySelectedCards == null) {
                selectedLabels[i] = new JLabel(new ImageIcon(getClass().getResource("../resources/blank.card.gif")));
            } else {
                selectedLabels[i] = new JLabel(
                    new ImageIcon(getClass().getResource("../resources/cards/" + alreadySelectedCards[i].toString() + ".gif"))
                );
    ...
    private class MainLabelListener extends MouseAdapter
    {
        ...
        public void mousePressed(MouseEvent e)
        {
            ...
            for (int i = 0; i < selectedStates.length; i++) {
                ...
                if (! selectedStates[i]) {
                    cardsLabels[row][column].setIcon(new ImageIcon(getClass().getResource("../resources/blank.card.gif")));
                    ...
                    selectedLabels[i].setIcon(
                        new ImageIcon(getClass().getResource("../resources/cards/" + newCard.toString() + ".gif"))
                    );
    ...
    private class SelectedLabelListener extends MouseAdapter
    {
        ...
        public void mousePressed(MouseEvent e)
        {
            ...
            selectedLabels[column].setIcon(new ImageIcon(getClass().getResource("../resources/blank.card.gif")));
            selectedStates[column] = false;
            
            cardsLabels[row][bigColumn].setIcon(
                new ImageIcon(getClass().getResource("../resources/cards/" + existingCard.toString() + ".gif"))
            );
