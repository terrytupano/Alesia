...
public final class GUI implements SimulationNotifiable {
	...
	private String blankCardPath = "../resources/blank.card.gif";
	...
	private GUI(PEDictionary language) {
		...
		mainframe.setIconImage(
				Toolkit.getDefaultToolkit().getImage(getClass().getResource("../resources/ace_spades_icon.jpg")));
	...
	private JPanel createChoicesPanel() {
		...
		String[] titles = {dictionary.getValue("table.handoptions.player"),
				dictionary.getValue("table.handoptions.handtype"), dictionary.getValue("table.handoptions.selection")};
	...
	private JPanel createResultsPanel() {
		...
		String[] titles = {dictionary.getValue("table.results.player"), dictionary.getValue("table.results.wins"),
				dictionary.getValue("table.results.loses"), dictionary.getValue("table.results.ties")};
	...
	private class StartSimulationListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				GUIUtilities.showErrorDialog(mainframe, "The simulator encountered an error: " + ex.getMessage(),
						"Simulator Error");
			}
		}
	...
	private void setCommunityCardsContent() {
		...
		if (currentCommunityCards[0] != null) {
			...
			flopCard1.setIcon(new ImageIcon(
					getClass().getResource("../resources/cards/" + currentCommunityCards[0].toString() + ".gif")));
			flopCard2.setIcon(new ImageIcon(
					getClass().getResource("../resources/cards/" + currentCommunityCards[1].toString() + ".gif")));
			flopCard3.setIcon(new ImageIcon(
					getClass().getResource("../resources/cards/" + currentCommunityCards[2].toString() + ".gif")));
		} else {
		...
		if (currentCommunityCards[3] != null) {
			...
			turnCard.setIcon(new ImageIcon(
					getClass().getResource("../resources/cards/" + currentCommunityCards[3].toString() + ".gif")));
		} else {
		...
		if (currentCommunityCards[4] != null) {
			...
			riverCard.setIcon(new ImageIcon(
					getClass().getResource("../resources/cards/" + currentCommunityCards[4].toString() + ".gif")));
		} else {
	...
	private void setGUIElementsDone(boolean stopped, SimulationFinalResult result) {
		...
		if (!stopped) {
			...
			statusBar.setText(dictionary.getValue("statusbar.done") + "(" + df.format(durationSeconds) + " "
	...
	private class ViewGraphListener implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
			ResultChartDialog rcd = new ResultChartDialog(mainframe, "Simulation Results Graph", simulator.getResult());
	...
	private HandType getSelectedHandType() {
		...
		switch (selectedType) {
			...
			case 1 :
				return HandType.RANGE;
			case 2 :
				return HandType.EXACTCARDS;
			default :
				return null;
		}
	}
