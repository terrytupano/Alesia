...
public class UoAHand {
   ...
   public UoAHand(String cs) {
      ...
      while(t.hasMoreTokens()) {
         ...
         if (s.length()==2) {
            UoACard c = new UoACard(s.charAt(0),s.charAt(1));
            if (c.getIndex() != UoACard.BAD_CARD)
               addCard(c);
         }
      }     
   }
    ...
    */
   public boolean addCard(UoACard c) {
    ...
    */
   public UoACard getCard(int pos) {
      if (pos < 1 || pos > cards[0]) return null;
      return new UoACard(cards[pos]);
   }
    ...
    * @param c the card to add
    */
   public void setCard(int pos, UoACard c) {
