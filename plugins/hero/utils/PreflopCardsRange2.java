 ...
 * @author Radu Murzea
 */
public class PreflopCardsRange2 {
	/**
	 ...
	 */
	public PreflopCardsRange2(int percentage) {
		setNewPercentage(percentage);
	}

	public static void main(String[] args) {
		PreflopCardsRange2 p2 = new PreflopCardsRange2();
		String prevhands = "";
		for (int per = 1; per <= 100; per++) {
			p2.setNewPercentage(per);
			String sql = "";
			String hands = "";
			for (int i = 0; i < 13; i++) {
				for (int j = 0; j < 13; j++) {
					if (p2.getValue(i, j)) {
						if (!prevhands.contains(p2.getCardAt(i, j))) {
							sql += "insert into preflopranges "
									+ "(rangeName, card, description, percentage, selected, ev) values "
									+ "('original', '" + p2.getCardAt(i, j)
									+ "', 'Original range from Pokerenleither', " + "15, false, " + (100 - per)
									+ ");\n";
							hands += p2.getCardAt(i, j) + " ";
							prevhands += p2.getCardAt(i, j) + " ";
						}
					}
				}
			}
			// System.out.println(100 - per + "\t" + hands);
	...
	public void setNewPercentage(int newPercentage) {
		...
		for (int i = 1; i <= newPercentage; i++) {
			for (Point c : PreflopCardsRange2.rangeProgressions[i]) {
				this.rangeSelections[c.x][c.y] = true;
			}
		}

		for (int i = newPercentage + 1; i < PreflopCardsRange2.rangeProgressions.length; i++) {
			for (Point c : PreflopCardsRange2.rangeProgressions[i]) {
