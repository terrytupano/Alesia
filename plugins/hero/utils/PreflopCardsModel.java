...
public class PreflopCardsModel {
	...
	/**
	 * new instace. the argumetn rangeName is name of a new oer existent preflop
	 * rage. if the DB dont contain such preflop range, this instance will create a
	 * fresh copy of preflopt list stored in <b>original</b>
	 * 
	...
	public PreflopCardsModel(String rangeName) {
		...
		if (preflopCards.size() == 0) {
			...
			// this.preflopCards = PreflopCards.where("rangeName = 'original' ORDER BY ev
			// DESC");
			// preflopCards.forEach(card -> card.set("rangeName", rangeName, "description",
			// rangeName, "percentage", 0,
	...
	/**
	 * Sets a new percentage for this range. This will overwrite only the cards
	 * inside of the selection. Cards outside of the specify procet, remaind equal.
	 * 
	...
	/**
	 * used for statistics purporse. this method update the <code>winnings</code>
	 * and <code>ev</code> fields preflopscards file.
	 * 
