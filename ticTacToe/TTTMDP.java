...
/**
 * This class specifies a full MDP for the Tic Tac Toe game, i.e. the Transition Function, T, and the Reward Function R
 * as per your lectures.
 * 
 * It does this by generating a set of {@link TransitionProb} objects for any given game/state and move, thus giving you
 * a probability distribution over next states and their associated rewards. The {@link #generateTransitions} method
 * does this. You will use an object of this class to implement your Value Iteration and Policy Iteration agents.
 * 
 * Note: this is only used for offline, planning methods such as Value Iteration and Policy Iteration, and, crucially,
 * NOT in Reinforcement Learning (e.g. Q-learning) where you should't assume access to such a model.
 * 
 * Note: This class assumes that the Value/Policy Iteration agents always play as 'X' (never 'O') - as does the
 * {@link ValueIterationAgent} class.
 * 
 * @author ae187
 */
...
public class TTTMDP {
	...
	/**
	 * Generates a list of TransitionProb objects containing game outcomes (source game, move, reward, target game)
	 * tuples paired with their probabilities. This essentially gives you a uniform probability distribution over all
	 * possible resulting game states and rewards when making move {@code m} in game {@code g}. This is implemented for
	 * you to use in your Value Iteration and Policy Iteration implementations.
	 * 
	 * @param g
	 * @param m
	 * @return
	...
	public List<TransitionProb> generateTransitions(Game g, Move m) {
		if (g.whoseTurn.getName() != m.who.getName())
			throw new IllegalArgumentException(
					"It's not " + m.who.getName() + "'s turn in game. This shouldn't happen!");

		if (m.who.getName() == 'O') {
			throw new IllegalStateException(
					"Value Iteration Agent is playing O. This will lead to unexpected results. It shouldn't happen.");
		}
		...
		try {
			...
			System.out.println(
					"WARNING: illegal move " + m + " tried when generating transitions. Returning empty list.");
